...
families<-unique(bootdf$Family_ID.reward)
...
for (r in 1:length(resamp_size)){
  for (n in 1:n_iter){
    ...
    for (f in 1:length(families)){
      sampled_ID<- sample(as.character(bootdf[which(bootdf$Family_ID.reward==families[f]),]$sub), 1)
...
write.csv(spearmancor_g3_O_reward_story, "/hcp_task_trait/figures/spearmancor_g3_O_unrelated_reward_story_check.csv", row.names = F)
