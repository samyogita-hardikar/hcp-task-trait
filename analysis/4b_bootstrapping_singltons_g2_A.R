...
families<-unique(bootdf$Family_ID.2back)
...
for (r in 1:length(resamp_size)){
  for (n in 1:n_iter){
    ...
    for (f in 1:length(families)){
      sampled_ID<- sample(as.character(bootdf[which(bootdf$Family_ID.2back==families[f]),]$sub), 1)
...
write.csv(spearmancor_g2_A_2back_motor, "/hcp_task_trait/figures/spearmancor_g2_A_unrelated_2back_motor_check.csv", row.names = F)
