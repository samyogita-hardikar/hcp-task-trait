...
#Agreeableness interaction of biggest + and - values

bootdf1<-df[which(df$cont=="2back"),]
...
resamplesubs<-list()
resampledf<-list()
spearmancor_g2_A_2back_motor<-data.frame()
...
for (r in 1:length(resamp_size)){
  for (n in 1:n_iter){
    ...
    resampledf[[n]]<-newdf
    cortest<-cor.test(c(resampledf[[n]]$corr_g2.2back -resampledf[[n]]$corr_g2.motor),
                      resampledf[[n]]$Agreeableness.2back)
    
    spearmancor_g2_A_2back_motor<-rbind(spearmancor_g2_A_2back_motor,
                                         cbind("g2", "Agreeableness", "2back_motor", nrow(resampledf[[n]]),
...
write.csv(spearmancor_g2_A_2back_motor, "/hcp_task_trait/figures/spearmancor_g2_A_whole_2back_motor_check.csv", row.names = F)
...
spearmancor_g2_A_c1_c2<-read.csv(paste0("/hcp_task_trait/figures/spearmancor_g2_A_whole_",
                                        c1, "_", c2, "_check.csv"), sep = ",")

spearmancor_g2_A_sum <- spearmancor_g2_A_c1_c2 %>%
...
p_boot<-ggplot(spearmancor_g2_A_sum, aes(x= as.factor(spearmancor_g2_A_sum$`as.numeric(V4)`),
                                         y= spearmancor_g2_A_sum$mean, 
                                         group=spearmancor_g2_A_sum$`as.factor(V3)`))+
  geom_ribbon(aes(x= as.factor(spearmancor_g2_A_sum$`as.numeric(V4)`), 
                  ymin= spearmancor_g2_A_sum$ic.min, ymax= spearmancor_g2_A_sum$ic.max,
                  fill = spearmancor_g2_A_sum$`as.factor(V3)`), alpha=0.2)+
  geom_ribbon(aes(x= as.factor(spearmancor_g2_A_sum$`as.numeric(V4)`), 
                  ymin= spearmancor_g2_A_sum$ic.min99, ymax= spearmancor_g2_A_sum$ic.max99,
                  fill = spearmancor_g2_A_sum$`as.factor(V3)`), alpha=0.3)+
  geom_ribbon(aes(x= as.factor(spearmancor_g2_A_sum$`as.numeric(V4)`),
                  ymin= spearmancor_g2_A_sum$ic.min95, ymax= spearmancor_g2_A_sum$ic.max95,
                  fill = spearmancor_g2_A_sum$`as.factor(V3)`), alpha=0.4)+
  ...
  scale_x_discrete(name= "Sample Size", breaks=levels(as.factor(spearmancor_g2_A_sum$`as.numeric(V4)`)),
                   labels= c("25",  "",  "",  "52",  "",  "",  "107", "", "",
                             "222", "", "", "459", "", "", "950"))+
